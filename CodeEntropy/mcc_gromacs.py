...
def parse_input():
	...
	# Usage
	inputParser.usage = "The script uses Gromacs' TRR (with coordinate and force at least) and TPR to compute entropy using the MCC method."

	# TOPOLOGY/STRUCTURE file (--tpr)
	inputParser.add_option("--tpr",
						   action="store",
						   dest  ="topology",
						   type  ="string",
						   help  ="Structure/topology file (.tpr)")

	# TRAJECTORY file (--trr)
	inputParser.add_option("--trr",
						   action="store",
						   dest  ="trajectory",
						   type  ="string",
...
def mcc_gromacs():
	...
	# Fetch topology and trajectory file names 
	options, args = parse_input()

	# READ ALL THE INPUT OPTIONS
	# filenames
	topolFile	 = options.topology
	trajFile	 = options.trajectory
	...
	mainMolecule, mainContainer = GromacsReader.read_gromacs_input(arg_tprFile = topolFile\
