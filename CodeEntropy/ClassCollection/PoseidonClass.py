...
class Poseidon_mp(Poseidon):
    def __init__(self ,container, start=0, end=-1, 
        ...
        p = ProcessingPool(nodes=thread)

        frame_list = []
        for frame in range(int(start), int(end), int(step)):
            ts = container.trajectory[frame]
            frame_list.append([np.array(ts.positions), np.array(ts.forces), np.array(ts.dimensions)])
        fi_partial = partial(frame_iteration, all_data, dimensions, startTime, verbosePrint, waterTuple, cutShell, excludedResnames)
...
def frame_iteration(all_data, dimensions, startTime, verbosePrint, waterTuple, cutShell, excludedResnames, frame):
    clearClass(all_data)
    print(f"frame = {frame}")
