...
def rotational_axis(iBead, arg_dataContainer, arg_numFrames):
    for iFrame in range(arg_numFrames):
        ...
        for iAtom in iBead.atomList:
            iBead.totalWeightedForces[iFrame,:] += arg_dataContainer.localForces[iFrame, iAtom]
            iBead.totalWeightedTorques[iFrame,:] += arg_dataContainer.localTorques[iFrame, iAtom]
...
def residue_UA(allSel, dataContainer, numFrames, heavyAtomArray, fScale, tScale, temper,  outfile, resid):
        baseResi = resid
...
if __name__ == "__main__":
    ...
    f = partial(residue_UA, allSel, dataContainer, numFrames, heavyAtomArray,1 ,1, 300, outfile)
